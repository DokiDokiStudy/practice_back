...
- ENV 레이어 3개 → 1개로 통합
- app.js 변경 시에도 ENV와 WORKDIR 레이어는 재사용
- CMD는 빌드 시 실행되지 않으므로 위치는 캐시에 큰 영향 없음
...
CMD ["npm", "start"]          # 항상 마지막 (컨테이너 실행 명령)
```
...
이제 이미지 레이어 캐시 최적화를 마스터했습니다!  
다음 챕터에서는 **애플리케이션 소스 코드에서 도커 이미지 빌드하기**를 학습하겠습니다.
